...
#    .env.example is the template, .env is YOUR local file with real values
cp .env.example .env

# 3. open .env in your editor and fill in real values:
#    - ANTHROPIC_API_KEY  (only required if you set USE_REAL_AI=true later)
#    - keep USE_REAL_AI=false initially (safe, $0 cost)
#    - PORT can stay 3000
...
Open the `.env` file you created in step 2 (NOT `.env.example`) and change:

```
USE_REAL_AI=true
```

Also make sure `ANTHROPIC_API_KEY` in your `.env` is a real key. You can create one at https://console.anthropic.com/settings/keys.

Then restart the server (Ctrl+C, then `npm run dev` again). One real run typically costs $0.05 to $0.10 with Haiku as configured.
...
3. **The agent runs**:
   - **Mock agent**: calls news tool, then calls financial tool, reflects, computes confidence, synthesizes a templated thesis. Useful for development.
...
| Haiku as default model | 5 to 10 times cheaper than Sonnet, fast enough for this kind of structured research. Easy to switch via `AIRA_MODEL`. |
...
| In-memory job store | Single-server submission. No Redis or SQL needed for the timebox. Behind a small `JobStore` class so it can be swapped. |
...
- **Real news / financial APIs**. Kept the data layer behind interchangeable interfaces with `USE_REAL_NEWS` and `USE_REAL_FINANCIAL` flags ready, but did not integrate `NewsAPI`, Alpha Vantage, or Yahoo Finance to avoid signup, key management, and rate-limit handling within the timebox.
- **Persistent job storage (Redis or SQL)**. `Map`-based store is enough for a single-process demo. The store is wrapped behind a small class so it can be replaced without touching agent code.
...
- **Caching of tool responses**. Listed as advanced or optional. Not built, but the seam exists in the tool interfaces.
...
- **Recurring or proactive analysis**. Listed as advanced or optional. Not in scope for one ticker.
...
1. **Real data sources**
   ...
   - Add per-tool retries with backoff and a small in-memory + Redis cache (5 to 15 minute TTL).
2. **Persistence**
...
5. **Reliability of the agent loop**
   ...
   - Self-correction: if confidence is below threshold or required fields are missing, re-prompt once with the gap.
6. **Observability**
